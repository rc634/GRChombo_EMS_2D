...
template <VariableType var_t> class AMRReductions
{
  ...
  public:
    ...
    // returns the position of maximum value for an interval of variables
    // (should say I haven't tested this for more than one variable)
    RealVect maxIndex(const Interval &a_vars) const;

    // returns position of maximum value for single variable
    RealVect maxIndex(const int a_var) const;

    // Functions below are adapted versions of computeMax in Chombo's 
    // computeNorm.H, adapted to find the position of the maximum
    RealVect computeMaxIndex(const Vector<LevelData<FArrayBox> *> &a_phi,
                             const Vector<int> &a_nRefFine,
                             const Interval a_comps, const int a_lBase) const;
    IntVect computeMaxIndex(const LevelData<FArrayBox> &a_phi,
