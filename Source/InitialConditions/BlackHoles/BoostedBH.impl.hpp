...
#elif CH_SPACEDIM == 2
    const data_t cos_theta = -m_params.center[1] / r;
#endif

    const data_t P_squared =
        D_TERM(pow(m_params.momentum[0], 2), +pow(m_params.momentum[1], 2),
...
{
    const data_t r = center_dist(coords);
    const data_t l[CH_SPACEDIM] = {
        (coords.x - m_params.center[0]) / r,
    ...
    const data_t l_dot_p =
        D_TERM(l[0] * m_params.momentum[0], +l[1] * m_params.momentum[1],
...
{
    data_t r = sqrt(D_TERM(pow(coords.x - m_params.center[0], 2),
