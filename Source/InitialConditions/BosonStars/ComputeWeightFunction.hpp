...
class ComputeWeightFunction
{
	...
	double q = m_params_BosonStar.mass_ratio;
        ...
        double profile_func1 = weightfunction.profile_chi((coords.x-q*separation/(q+1))*cosh(rapidity), coords.y+q*impact_parameter/(q+1.), 0., radius_width1);
        double profile_func2 = weightfunction.profile_chi((coords.x+separation/(q+1))*cosh(-rapidity2), coords.y- impact_parameter/(q+1.), 0., radius_width2);
