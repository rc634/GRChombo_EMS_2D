...
template <> struct ParmParseTranslator<std::string>
{
    static constexpr ParmParse::PPType pp_type = ParmParse::ppString;
};
...
class GRParmParse : public ParmParse
{
  public:
    ...
    /// Loads a string from the parameter file
    template <class data_t>
    void load(const char *name, std::string string = "./") const
    {
        get(name, string);
    }
