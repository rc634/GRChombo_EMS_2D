...
#ifndef PUNCTURETRACKERTAGGINGCRITERION_HPP_
#define PUNCTURETRACKERTAGGINGCRITERION_HPP_
...
class PunctureTrackerTaggingCriterion
{
  protected:
    const double m_dx;
    const int m_level;
  ...
  public:
    ...
    // The constructor
    PunctureTrackerTaggingCriterion(
            ...
            if (puncture_separation > m_puncture_min_separation)
            {
                ...
                for (int ipuncture = 0; ipuncture < 2; ++ipuncture)
                {
                    ...
                    if ((m_level <= m_puncture_max_levels[ipuncture]))
                    {
                        ...
                        // finest level
                        const double factor =
                            m_puncture_masses[ipuncture] * 2 *
                            pow(1.5,
                                m_puncture_max_levels[ipuncture] - m_level);
                        auto regrid = simd_compare_lt(r, factor);
                        criterion = simd_conditional(regrid, 100.0, criterion);
                    }
                }
            }
